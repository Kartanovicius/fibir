...
export default function Timeline() {
    ...
    return (
        <div className="container col-span-2 border-r-0">
            <AddPost/>
            {!allData ? (
                <Skeleton circle={false} count={1} height={50} className="mt-5"/>
            ) : allData.length > 0 ? (
                <div className="flex flex-row bg-black-light shadow-lg` p-2 pl-5 mb-8 text-white flex-wrap">
                    ...
                    {allData.map((filter) => (
                        <button className="p-2 hover:bg-blue-medium mr-3"
                                type="button"
                                onClick={handleClick}
                                value={filter.name}>
                            {filter.name}
